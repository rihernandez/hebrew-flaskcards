...
const ALLOWED_KEYS = new Set(['darkMode', 'selected_language', 'wod_seen', 'wod_date', 'wod_id']);
...
export class StateService {
  ...
  private assertAllowed(key: string) {
    if (!ALLOWED_KEYS.has(key)) {
      throw new BadRequestException(`Key not allowed in preferences-ms: ${key}`);
    }
  }

  async list(userId: string) {
    return this.model.find({ userId }).select('key value updatedAt -_id').lean();
  }
